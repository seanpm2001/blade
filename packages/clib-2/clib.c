...
static b_value clib_ffi_cdata_read_dim(b_vm *vm, clib_ffi_cdata *cdata, const b_value *offset, b_value *rv) {
	clib_ffi_type  *type = CLIB_FFI_TYPE(cdata->type);
...
static void clib_ffi_cdata_write_dim(clib_ffi_cdata *cdata, const b_value *offset, b_value *value) {
	clib_ffi_type  *type = CLIB_FFI_TYPE(cdata->type);
	long       dim;
	void           *ptr;
	clib_ffi_flags  is_const;
...
static bool clib_ffi_ctype_name(b_vm *vm, clib_ffi_ctype_name_buf *buf, const clib_ffi_type *type) {
		...
		switch (type->kind) {
			...
			case CLIB_FFI_TYPE_ENUM:
				if (type->enumeration.tag_name) {
					clib_ffi_ctype_name_prepend(buf, type->enumeration.tag_name->chars, type->enumeration.tag_name->length);
				} else {
          ...
          int length = snprintf(NULL, 0, "%ld", type->array.length);
          char *s = ALLOCATE(char, length);
          sprintf(s + sizeof(s) - 1, "%ld", type->array.length);
			...
			case CLIB_FFI_TYPE_STRUCT:
				if (type->attr & CLIB_FFI_ATTR_UNION) {
					if (type->record.tag_name) {
						clib_ffi_ctype_name_prepend(buf, type->record.tag_name->chars, type->record.tag_name->length);
					} else {
						clib_ffi_ctype_name_prepend(buf, "<anonymous>", sizeof("<anonymous>")-1);
					}
					name = "union ";
...
static CLIB_COLD void clib_ffi_pass_incompatible(b_vm *vm, b_value *arg, clib_ffi_type *type, uint32_t n, b_value *execute_data) {
	clib_ffi_ctype_name_buf buf1, buf2;

	buf1.start = buf1.end = buf1.buf + ((MAX_TYPE_NAME_LEN * 3) / 4);
	if (!clib_ffi_ctype_name(vm, &buf1, type)) {
